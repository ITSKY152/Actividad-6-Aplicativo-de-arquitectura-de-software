...
import ListUsers from './components/ListNotes'
...
function App() {
  ...
  const deleteUser = (user) => {
    ...
    fetch(`http://localhost:4000/notas/${user.id}`, requestOptions)
  ...
  const GetUsersByEmail = async (id) => {
    if (id === "") {
      GetUsers()
    }
    else {
      ...
      fetch(`http://localhost:4000/usuarios/${id}`, requestOptions)
        .then(response => response.json())
        .then(data => {
          setusuarios(data)
        });
